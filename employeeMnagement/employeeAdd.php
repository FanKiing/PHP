...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($employees as $employee) {
        if ($employee['SN'] == $SN) {
            array_push($errors, 'Serial number already exists');
            break;
        }
    }
    if (empty($errors)) {
...
<button onclick="window.location.href='employeeDisplay.php'">View Employees</button>
    ...
    <form action="employeeAdd.php" method="post" name="myForm" enctype="multipart/form-data">
        <label for="SN">Serial number</label>
        <input type="number" id="SN" name="SN" min="1000" max="9999" required>
        ...
        <input type="file" name="photo" accept="image/jpg,image/png,image/gif,image/jpeg" required>

        <label for="cv">CV</label>
        <input type="file" name="cv" accept="application/pdf" required>

        <label for="acc">LinkedIn account</label>
        <input type="url" id="acc" name="acc" required>
